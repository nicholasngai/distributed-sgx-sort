...
static int distributed_quickselect_helper(void *arr, size_t local_length,
    ...
    for (int i = 0; i < world_size; i++) {
        bool is_ready;
        if (i != world_rank) {
            ret = mpi_tls_recv_bytes(&is_ready, sizeof(is_ready), i, 0);
            if (ret) {
                handle_error_string(
    ...
    if (world_rank == master_rank) {
        ...
        ret = node_decrypt(key, &pivot_node, arr + left * SIZEOF_ENCRYPTED_NODE,
                left + local_start);
        if (ret) {
            handle_error_string("Error decrypting node %lu",
                    left + local_start);
        ...
        ret = mpi_tls_recv_bytes(&pivot, sizeof(pivot), master_rank, 0);
        if (ret) {
    ...
    while (partition_left < partition_right) {
        ...
        case PARTITION_SCAN_LEFT:
            ...
            ret = node_decrypt(key, &left_node,
        ...
        case PARTITION_SCAN_RIGHT:
            ...
            ret = node_decrypt(key, &right_node,
            ...
            if (node_sample_comparator(&right_node, &pivot) < 0) {
                ret = node_encrypt(key, &left_node,
                        arr + (partition_right - 1) * SIZEOF_ENCRYPTED_NODE,
                        partition_right - 1 + local_start);
                if (ret) {
                    handle_error_string("Error encrypting node %lu",
                ...
                ret = node_encrypt(key, &right_node,
                        arr + partition_left * SIZEOF_ENCRYPTED_NODE,
                        partition_left + local_start);
                if (ret) {
                    handle_error_string("Error encrypting node %lu",
    ...
    if (world_rank == master_rank) {
        ...
        ret = node_encrypt(key, &node, arr + left * SIZEOF_ENCRYPTED_NODE,
                left + local_start);
        if (ret) {
            handle_error_string("Error encrypting node %lu",
                    left + local_start);
        ...
        ret = node_encrypt(key, &pivot_node,
                arr + (partition_right - 1) * SIZEOF_ENCRYPTED_NODE,
                partition_right - 1 + local_start);
        if (ret) {
            handle_error_string("Error encrypting node %lu",
    ...
    if (world_rank == master_rank) {
        ...
        for (int i = 0; i < world_size; i++) {
            if (i != world_rank) {
                ...
                ret = mpi_tls_recv_bytes(&remote_partition_right,
        ...
        for (int i = 0; i < world_size; i++) {
            if (i != world_rank) {
                ret = mpi_tls_send_bytes(&cur_pivot, sizeof(cur_pivot), i,
                        0);
                if (ret) {
                    handle_error_string(
        ...
        ret = mpi_tls_send_bytes(&partition_right, sizeof(partition_right),
                master_rank, 0);
        if (ret) {
            handle_error_string(
        ...
        ret = mpi_tls_recv_bytes(&cur_pivot, sizeof(cur_pivot), master_rank,
                0);
        if (ret) {
            handle_error_string(
    ...
    if (geq_target_idx > 0) {
        ...
        ret = distributed_quickselect_helper(arr, local_length, local_start,
                targets, samples, geq_target_idx, left, partition_right);
        if (ret) {
            goto exit;
        }
    }
    if (gt_target_idx < num_targets) {
        ...
        ret = distributed_quickselect_helper(arr, local_length, local_start,
                ...
                num_targets - gt_target_idx, partition_left, right);
        if (ret) {
            goto exit;
        }
    }
...
#pragma GCC diagnostic pop
    ...
    int ret =
        distributed_quickselect_helper(arr, local_length, local_start, targets,
                samples, num_targets, 0, local_length);
    if (ret) {
...
static int distributed_sample_partition(void *arr, void *out,
...
int bucket_sort(void *arr, size_t length, size_t num_threads) {
    ...
    ret =
        distributed_sample_partition(arr,
                arr + src_local_length * 2 * SIZEOF_ENCRYPTED_NODE,
                compress_len, local_start, length);
    if (ret) {
        handle_error_string("Error in distributed sample partitioning");
        goto exit;
    }

    // TODO Sort local partitions.
