...
for e in 32 16 8 4 2 1; do
    ...
    for a in bitonic bucket orshuffle; do
        ...
        for s in 256 4096 65536 1048576 16777216; do
            for t in 1 2 4; do
                if [ "$a" = 'bitonic' ]; then
                    output_filename="$BENCHMARK_DIR/$a-sgx2-enclaves$e-chunked$BITONIC_CHUNK_SIZE-elemsize128-size$s-threads$t.txt"
                elif [ "$a" = 'bucket' ]; then
                    output_filename="$BENCHMARK_DIR/$a-sgx2-enclaves$e-bucketsize$BUCKET_SIZE-elemsize128-size$s-threads$t.txt"
                elif [ "$a" = 'orshuffle' ]; then
                    output_filename="$BENCHMARK_DIR/$a-sgx2-enclaves$e-elemsize128-size$s-threads$t.txt"
                else
